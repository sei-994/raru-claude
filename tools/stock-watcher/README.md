...
`.env.example` に入れてある iPhone 18 Pro Max 256GB SIMフリーの品番
（**2026-09-18 に `--doctor` で4つとも実在を確認済み**）:
...
別の容量・機種を狙う場合は Apple 自身のページから引き直してください
（品番が誤っていれば `--doctor` の `[2/4] 品番の確認` が `✗` を出します）:
