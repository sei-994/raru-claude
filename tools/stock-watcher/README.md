...
品番のままだと読みにくいので、表示名と買取価格を `prices.json` で設定できます。**任意です**
（無くても監視は動きます。その場合は品番がそのまま表示され、利益計算は行われません）。

```bash
cp prices.example.json prices.json
open -e prices.json      # labels / cost / buyers を埋める
node apple-stock.js --profit
```

```
機種                    仕入        じゃんぱら    ゲオ          最高値との差
------------------------------------------------------------------------------
256GB ブラック          ¥234,800    ¥228,000      ¥231,000      −3,800  ゲオ
512GB バーガンディ      ¥274,800    -             ¥281,000      +6,200  ゲオ
```
...
```
🟢 在庫が出ました
• 渋谷（渋谷区） — 256GB ブラック, 512GB バーガンディ / 本日 受け取り可能

💰 買取見込み（最高値の店）
256GB ブラック　仕入 ¥234,800 → ゲオ ¥231,000（−3,800）
512GB バーガンディ　仕入 ¥274,800 → ゲオ ¥281,000（+6,200）
```

- `cost` = Appleでの支払額（税込）、`buyers` = 買取店ごとの買取価格。どちらも**手入力**です
- 金額が `0` の行は未設定として扱われ、利益は出ません
- **買取価格の自動取得には対応していません。** 主要買取店に公開APIがなく、
  スクレイピングは各社の規約に触れる可能性があるためです。相場は自分で更新してください
...
| 品番 | 容量・色 | 確度 |
|---|---|---|
| `MJX54J/A` | 256GB ブラック | **確認済** `--doctor` で実在確認 |
| `MJX64J/A` | 256GB シルバー | **確認済** |
| `MJX74J/A` | 256GB バーガンディ | **確認済** |
| `MJX84J/A` | 256GB グレイシャー | **確認済** |
| `MJXC4J/A` | 512GB バーガンディ | 小売店の商品ページで確認 |
| `MJXA4J/A` | 512GB ブラック | **推測**（256GBの採番規則から） |
| `MJXB4J/A` | 512GB シルバー | **推測** |
| `MJXD4J/A` | 512GB グレイシャー | **推測** |

**推測の3件は `--doctor` が判定します。** `[2/4] 品番の確認` で `✗` が出た品番を
`.env` から消してください。あるいは Apple 自身のページから引き直せます:
